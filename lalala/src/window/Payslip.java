...
public class Payslip extends JInternalFrame implements ActionListener {
	...
	public Payslip(JFrame getParentFrame)

	{
		...
		Lblcollege2 = new JLabel("Pay Slip Report");
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (source == PrintBtn) {
			Print prn = new Print();
			prn.fill_data(sEmp_Name1, sEmp_Name2, Emp_Month, Emp_Year, sEmp_Code, sDate, sEmp_Desi, sBasic_Pay,
					sDA_Allow, sHRA_Allow, sWA_Allow, sGPF_Dedu, sIT_Dedu, sGIS_Dedu, sPF_Dedu, sLIC_Dedu, sAllow,
					sDedu, sNet_Salary);
			...
			if (!Desi.equals("")) {
				...
				while ((line = br.readLine()) != null) {
					...
					if (d[1].equals(Desi)) {
						...
						vBasic_Pay = Integer.parseInt(d[2].trim());
						sDA = d[3].trim();
						sHRA = d[4].trim();
						sWA = d[5].trim();
						sGPF = d[6].trim();
						...
						sPF = d[9].trim();
						sLIC = d[10].trim();
						DA_Value = Integer.parseInt(d[11].trim());
						HRA_Value = Integer.parseInt(d[12].trim());
						WA_Value = Integer.parseInt(d[13].trim());
						GPF_Value = Integer.parseInt(d[14].trim());
						IT_Value = Integer.parseInt(d[15].trim());
						GIS_Value = Integer.parseInt(d[16].trim());
						PF_Value = Integer.parseInt(d[17].trim());
						LIC_Value = Integer.parseInt(d[18].trim());

...
public class Setting extends JInternalFrame implements ActionListener, ItemListener {
	...
	public Setting(JFrame getParentFrame) {
...
//		uncheck_true();
	...
	public void add_record() {
		try {
			...
			if (!sCategory_Type.equals("") && !sCategory_Name.equals("") && !sBasic_Pay.equals("")
					&& !sDA_Allow.equals("") && !sHRA_Allow.equals("") && !sWA_Allow.equals("") && !sGPF_Dedu.equals("")
					&& !sIT_Dedu.equals("") && !sGIS_Dedu.equals("") && !sPF_Dedu.equals("") && !sLIC_Dedu.equals("")) {

				System.out.println("Category Name :" + sCategory_Name);
				try {
					String line;
					BufferedReader br = new BufferedReader(
							new FileReader(System.getProperty("user.dir") + "/bin/setting.csv"));
					while ((line = br.readLine()) != null) {
						if (line.length() == 0)
							break;
						String[] d = line.split(";");
						if (d[1].equals(sCategory_Name)) {
				...
				String temp = sCategory_Type + ";" + sCategory_Name + ";" + sBasic_Pay + ";" + sDA + ";" + sHRA + ";"
						+ sWA + ";" + sGPF + ";" + sIT + ";" + sGIS + ";" + sPF + ";" + sLIC + ";" + sDA_Allow + ";"
						+ sHRA_Allow + ";" + sWA_Allow + ";" + sGPF_Dedu + ";" + sIT_Dedu + ";" + sGIS_Dedu + ";"
						+ sPF_Dedu + ";" + sLIC_Dedu + "\n";
				Files.write(Paths.get(System.getProperty("user.dir") + "/bin/setting.csv"), temp.getBytes(),
	...
	public void fill_form(String name) {
		...
		try {
			// Category_Type;Category_Name;Basic_Pay;DA;HRA;WA;GPF;IT;GIS;PF;LIC;DA_Allow;HRA_Allow;WA_Allow;GPF_Dedu;IT_Dedu;GIS_Dedu;PF_Dedu;LIC_Dedu
			...
			while ((line = br.readLine()) != null) {
				...
				if (d[1].equals(name)) {
					...
					sCategory_Type += d[0].trim();
					sCategory_Name = d[1].trim();
					sBasic_Pay = d[2].trim();
					sDA = d[3].trim();
					sHRA = d[4].trim();
					sWA = d[5].trim();
	...
	public void update_record() {
		try {
			...
			if (!sCategory_Name.equals("") && !sBasic_Pay.equals("") && !sDA_Allow.equals("") && !sHRA_Allow.equals("")
					&& !sWA_Allow.equals("") && !sGPF_Dedu.equals("") && !sIT_Dedu.equals("") && !sGIS_Dedu.equals("")
					&& !sPF_Dedu.equals("") && !sLIC_Dedu.equals(""))

			{
				...
				try {
					...
					while ((line = br.readLine()) != null) {
						...
						String[] d = line.split(";");
						if (d[1].equals(sCategory_Name)) {
							String temp = sCategory_Type + ";" + sCategory_Name + ";" + sBasic_Pay + ";" + sDA + ";"
									+ sHRA + ";" + sWA + ";" + sGPF + ";" + sIT + ";" + sGIS + ";" + sPF + ";" + sLIC
									+ ";" + sDA_Allow + ";" + sHRA_Allow + ";" + sWA_Allow + ";" + sGPF_Dedu + ";"
									+ sIT_Dedu + ";" + sGIS_Dedu + ";" + sPF_Dedu + ";" + sLIC_Dedu;
							db += temp + "\n";
							updated = true;
						} else
							db += line + "\n";
				...
				if (updated) {
					Files.write(Paths.get(System.getProperty("user.dir") + "/bin/setting.csv"), db.getBytes(),
			...
			try {
...

				{
					...
					if (deleted) {
						...
						dialogtype = JOptionPane.INFORMATION_MESSAGE;
						JOptionPane.showMessageDialog((Component) null, dialogmessage, dialogs, dialogtype);
						Cat_Name.removeItem(sCategory_Name);
					} else {
